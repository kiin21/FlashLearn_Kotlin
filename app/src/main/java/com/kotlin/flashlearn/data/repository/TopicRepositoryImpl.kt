...
class TopicRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAllTopics(): Result<List<Topic>> {
        return try {
            val request = NeonSqlRequest(
                query = "SELECT id, name, description, icon_type, is_system_topic, created_by FROM topics ORDER BY is_system_topic DESC, name ASC"
            )
    ...
    override suspend fun getUserTopics(userId: String): Result<List<Topic>> {
        return try {
            val request = NeonSqlRequest(
                query = "SELECT id, name, description, icon_type, is_system_topic, created_by FROM topics WHERE created_by = \$1 ORDER BY name ASC",
    ...
    override suspend fun getTopicById(topicId: String): Result<Topic?> {
        return try {
            val request = NeonSqlRequest(
                query = "SELECT id, name, description, icon_type, is_system_topic, created_by FROM topics WHERE id = \$1",
    ...
    override suspend fun createTopic(topic: Topic): Result<Topic> {
        return try {
            ...
            val request = NeonSqlRequest(
                query = """
                    INSERT INTO topics (id, name, description, icon_type, is_system_topic, created_by)
                    VALUES ($1, $2, $3, $4, $5, $6)
                    RETURNING id, name, description, icon_type, is_system_topic, created_by
                """.trimIndent(),
                params = listOf(
                    id,
                    topic.name,
    ...
    override suspend fun searchTopics(query: String): Result<List<Topic>> {
        return try {
            val request = NeonSqlRequest(
                query = "SELECT id, name, description, icon_type, is_system_topic, created_by FROM topics WHERE name ILIKE \$1 ORDER BY name ASC",
                params = listOf("%$query%")
            )
            val response = neonSqlApi.executeQuery(
                connectionString = CONNECTION_STRING,
